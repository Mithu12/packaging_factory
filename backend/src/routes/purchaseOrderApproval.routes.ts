...
const canSubmit = (req: any, res: any, next: any) => {
  ...
  if (!ApprovalMediator.canSubmit(userRole)) {
    return serializeErrorResponse(res, 'Insufficient permissions to submit for approval', 403);
  }
  next();
};
...
const canApprove = (req: any, res: any, next: any) => {
  ...
  if (!ApprovalMediator.canApprove(userRole)) {
    return serializeErrorResponse(res, 'Insufficient permissions to approve', 403);
  }
  next();
};
