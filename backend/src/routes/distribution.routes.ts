...
router.get('/centers',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const result = await DistributionCenterMediator.getDistributionCenters(req.query as any);
...
router.get('/centers/stats',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const stats = await DistributionCenterMediator.getDistributionCenterStats();
...
router.put('/centers/:id',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      MyLogger.error(action, error, { centerId, updates: Object.keys(req.body) });
      throw error;
    }
  })
);
...
router.get('/locations',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const result = await ProductLocationMediator.getProductLocations(req.query as any);
...
router.get('/locations/allocation-view',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const allocations = await ProductLocationMediator.getProductAllocationView();
...
router.get('/locations/product/:productId',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const locations = await ProductLocationMediator.getProductLocationsByProduct(productId);
...
router.put('/locations/:id',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      MyLogger.error(action, error, { locationId, updates: Object.keys(req.body) });
      throw error;
    }
  })
);
...
router.post('/locations/:id/adjust-stock',
  authenticate,
  auditMiddleware,
  requirePermission(PERMISSIONS.INVENTORY_ADJUST),
  expressAsyncHandler(async (req, res, next) => {
...
router.post('/locations/allocate',
  ...
  expressAsyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      const allocation = await ProductLocationMediator.allocateProduct(req.body);
