...
export const createPurchaseOrderSchema = Joi.object({
  ...
  payment_terms: Joi.string().max(50).optional().messages({
    "string.max": "Payment terms cannot exceed 50 characters",
  }),

  delivery_terms: Joi.string().max(50).optional().messages({
  ...
  project: Joi.string().max(100).optional().messages({
...
export const updatePurchaseOrderSchema = Joi.object({
  ...
  expected_delivery_date: Joi.date().iso().optional().messages({
    "date.base": "Expected delivery date must be a valid date",
    "date.format": "Expected delivery date must be in ISO format (YYYY-MM-DD)",
  }),

  actual_delivery_date: Joi.date().iso().optional().messages({
...
export const updatePurchaseOrderStatusSchema = Joi.object({
  status: Joi.string()
    ...
    .messages({
      "any.only":
        "Status must be one of: draft, pending, approved, sent, partially_received, received, cancelled",
...
export const receiveGoodsSchema = Joi.object({
  line_items: Joi.array()
    .items(
      Joi.object({
        line_item_id: Joi.number().integer().positive().required().messages({
          "number.base": "Line item ID must be a number",
          "number.integer": "Line item ID must be an integer",
          "number.positive": "Line item ID must be positive",
        ...
        received_quantity: Joi.number().positive().required().messages({
...
export const purchaseOrderQuerySchema = Joi.object({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "Sort by must be one of: id, po_number, order_date, expected_delivery_date, total_amount, created_at, updated_at",
    }),
