...
export const createCustomerOrderSchema = Joi.object({
    ...
    pr_no: Joi.string().max(50).optional().allow('', null),
    po_number: Joi.string().max(50).optional().allow('', null),
    po_date: Joi.date().iso().optional().allow(null).empty(''),
...
export const orderQuerySchema = Joi.object({
    ...
    status: Joi.string().valid('', 'draft', 'pending', 'quoted', 'approved', 'rejected', 'in_production', 'completed', 'partially_shipped', 'shipped', 'cancelled').optional(),
    priority: Joi.string().valid('low', 'medium', 'high', 'urgent').optional(),
    factory_customer_id: Joi.number().integer().positive().optional(),
