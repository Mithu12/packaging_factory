import { SalesRepPayment, CreatePaymentRequest } from "../../types";
  ...
  async createPayment(
    ...
    try {
      ...
      if (data.amount > invoice.balance_amount) {
        throw new Error(
          `Payment amount (${data.amount}) cannot exceed outstanding balance (${invoice.balance_amount})`
        );
      }
