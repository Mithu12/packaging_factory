...
import { CreateCategoryRequest, CreateSubcategoryRequest, Category, Subcategory } from "@/types/category";
...
class AddCategoryMediator {
    ...
    async createSubcategory(data: CreateSubcategoryRequest): Promise<Subcategory> {
        ...
        const client = await pool.connect();
        try {
            MyLogger.info(action, { subcategoryName: data.name, categoryId: data.category_id })
            ...
            const result = await client.query(query, values);
            MyLogger.success(action, { subcategoryId: result.rows[0].id, subcategoryName: result.rows[0].name })
            return result.rows[0];
        } catch (error: any) {
            MyLogger.error(action, error, { subcategoryName: data.name, categoryId: data.category_id })
            if (error.code === '23505') { // Unique violation
