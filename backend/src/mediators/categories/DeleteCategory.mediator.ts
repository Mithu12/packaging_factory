...
class DeleteCategoryMediator {
    ...
    async deleteCategory(id: number): Promise<void> {
        ...
        try {
            ...
            MyLogger.info('Delete Subcategories', { categoryId: id, message: 'Deleted all subcategories' })

            // Delete the category
            const result = await client.query('DELETE FROM categories WHERE id = $1', [id]);

            if (result.rowCount === 0) {
                MyLogger.warn(action, { categoryId: id, message: 'Category not found for deletion' })
    ...
    async deleteSubcategory(id: number): Promise<void> {
        ...
        try {
            ...
            if (result.rowCount === 0) {
                MyLogger.warn(action, { subcategoryId: id, message: 'Subcategory not found for deletion' })
