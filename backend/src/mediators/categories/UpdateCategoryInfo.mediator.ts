import { Category, Subcategory, UpdateCategoryRequest, UpdateSubcategoryRequest } from "@/types/category";
...
class UpdateCategoryInfoMediator {
    ...
    async updateCategory(id: number, data: UpdateCategoryRequest): Promise<Category> {
        ...
        try {
            MyLogger.info(action, { categoryId: id, updateFields: Object.keys(data) })
    ...
    // Update subcategory
    async updateSubcategory(id: number, data: UpdateSubcategoryRequest): Promise<Subcategory> {
        let action = 'Update Subcategory'
        const client = await pool.connect();
        try {
            MyLogger.info(action, { subcategoryId: id, updateFields: Object.keys(data) })
            ...
            if (category_id) {
                ...
                if (categoryCheck.rows.length === 0) {
                    MyLogger.warn(action, { categoryId: category_id, message: 'Parent category not found' })
            ...
            if (error.code === '23505') { // Unique violation
                throw createError('Subcategory with this name already exists in this category', 409);
            }
            throw error;
        } finally {
            client.release();
