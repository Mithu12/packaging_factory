import { Category, Subcategory, CategoryQueryParams, SubcategoryQueryParams, CategoryStats } from "@/types/category";
...
class GetCategoryInfoMediator implements MediatorInterface {
    ...
    async getCategoryList(params: CategoryQueryParams): Promise<{
        ...
        try {
            ...
            MyLogger.success(action, { total, page, limit, returnedCount: categoriesResult.rows.length })
            return {
    ...
    async getCategoryById(id: number): Promise<Category> {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                MyLogger.warn(action, { categoryId: id, message: 'Category not found' })
            ...
            MyLogger.success(action, { categoryId: id, categoryName: category.name, subcategoriesCount: subcategoriesResult.rows.length })
            return category;
        } catch (error: any) {
    ...
    async getSubcategoryList(params: SubcategoryQueryParams): Promise<{
        ...
        try {
            ...
            queryParams.push(limit, offset);
            const subcategoriesResult = await client.query(subcategoriesQuery, queryParams);

            MyLogger.success(action, { total, page, limit, returnedCount: subcategoriesResult.rows.length })
            return {
    ...
    async getSubcategoryById(id: number): Promise<Subcategory> {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                MyLogger.warn(action, { subcategoryId: id, message: 'Subcategory not found' })
                throw createError('Subcategory not found', 404);
            }

            MyLogger.success(action, { subcategoryId: id, subcategoryName: result.rows[0].name })
    ...
    async getCategoryStats(): Promise<CategoryStats> {
        ...
        try {
            ...
            const categoryStats = {
                ...
                categories_with_subcategories: parseInt(stats.categories_with_subcategories),
                average_subcategories_per_category: parseFloat(stats.average_subcategories_per_category)
            };
    ...
    async searchCategories(query: string, limit: number = 10): Promise<Category[]> {
        ...
        try {
            ...
            MyLogger.success(action, { query, limit, resultsCount: result.rows.length })
            return result.rows;
        } catch (error: any) {
    ...
    async searchSubcategories(query: string, limit: number = 10): Promise<Subcategory[]> {
