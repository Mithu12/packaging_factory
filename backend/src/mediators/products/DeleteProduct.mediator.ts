...
export class DeleteProductMediator {
    ...
    static async hardDeleteProduct(id: number): Promise<void> {
        ...
        try {
            ...
            if (parseInt(references.po_items) > 0 || parseInt(references.inventory_transactions) > 0) {
                MyLogger.warn(action, { 
                    productId: id, 
                    message: 'Product has references in other tables',
                    references: references
                });
                throw new Error('Cannot delete product: it has references in purchase orders or inventory transactions');
            }
    ...
    static async checkProductReferences(id: number): Promise<{
        ...
        try {
            ...
            const hasReferences = referencesCount.purchase_order_items > 0 || referencesCount.inventory_transactions > 0;
