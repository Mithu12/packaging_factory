...
class GetSupplierInfoMediator implements MediatorInterface {
    ...
    async getSupplierList(params: SupplierQueryParams): Promise<{
        ...
        try {
          ...
          supplier_code ILIKE $${paramIndex}
            ...
            MyLogger.success(action, { total, page, limit, returnedCount: suppliersResult.rows.length })
            return {
    ...
    async getSupplierById(id: number): Promise<Supplier> {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                MyLogger.warn(action, { supplierId: id, message: 'Supplier not found' })
    ...
    async getSupplierCategories(): Promise<string[]> {
        ...
        try {
            ...
            MyLogger.success(action, { categoriesCount: categories.length, categories })
            return categories;
        } catch (error: any) {
            MyLogger.error(action, error)
            throw error;
