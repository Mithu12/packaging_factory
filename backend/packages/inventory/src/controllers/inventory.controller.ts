...
import { InventoryMediator } from "../mediators/InventoryMediator";
...
class InventoryController {
  async getInventoryItems(
    ...
    try {
      ...
      const inventoryItems = await InventoryMediator.getInventoryItems(
        req.query as any
      );
  ...
  async getStockMovements(
    ...
    try {
      MyLogger.info(action, { query: req.query });
      const movements = await InventoryMediator.getStockMovements(
        req.query as any
      );
  ...
  async getInventoryItemById(
    ...
    try {
      ...
      MyLogger.success(action, {
        inventoryItemId: id,
        productName: (inventoryItem as any).product_name,
      });
