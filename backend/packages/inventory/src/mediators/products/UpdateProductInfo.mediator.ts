...
import { UpdateProductRequest, Product } from "../../../../src/types/product";
  ...
  static async updateProduct(
    ...
    try {
      ...
      if (productData.sku && productData.sku !== existingProduct.sku) {
        const skuExists = await AddProductMediator.checkSkuExists(
          productData.sku,
          id
        );
        if (skuExists) {
      ...
      if (
        ...
        const barcodeExists = await AddProductMediator.checkBarcodeExists(
          productData.barcode,
          id
        );
        if (barcodeExists) {
