...
-- Note: If operators table exists, it will be renamed to employees and extended with new columns
-- This migration handles the transition from operators to employees table
...
-- Create trigger for employees updated_at (simple creation - will be replaced if exists)
...
-- Create trigger for employee_contracts updated_at (simple creation - will be replaced if exists)
...
-- Add foreign key constraints for employees table (after all referenced tables are created)
ALTER TABLE employees ADD CONSTRAINT fk_employees_factory FOREIGN KEY (factory_id) REFERENCES factories(id) DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE employees ADD CONSTRAINT fk_employees_user FOREIGN KEY (user_id) REFERENCES users(id) DEFERRABLE INITIALLY DEFERRED;
...
ALTER TABLE employees ADD CONSTRAINT fk_employees_department FOREIGN KEY (department_id) REFERENCES departments(id) DEFERRABLE INITIALLY DEFERRED;

-- Add foreign key constraints for all tables (after all tables are created)

-- Departments table constraints
ALTER TABLE departments ADD CONSTRAINT fk_departments_manager FOREIGN KEY (manager_id) REFERENCES employees(id) DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE departments ADD CONSTRAINT fk_departments_parent FOREIGN KEY (parent_department_id) REFERENCES departments(id) DEFERRABLE INITIALLY DEFERRED;

-- Designations table constraints
ALTER TABLE designations ADD CONSTRAINT fk_designations_department FOREIGN KEY (department_id) REFERENCES departments(id) DEFERRABLE INITIALLY DEFERRED;

-- Employee contracts table constraints
ALTER TABLE employee_contracts ADD CONSTRAINT fk_employee_contracts_employee FOREIGN KEY (employee_id) REFERENCES employees(id) ON DELETE CASCADE;
ALTER TABLE employee_contracts ADD CONSTRAINT fk_employee_contracts_created_by FOREIGN KEY (created_by) REFERENCES users(id) DEFERRABLE INITIALLY DEFERRED;

-- Update existing references to point to employees table
ALTER TABLE work_order_material_consumptions ADD CONSTRAINT fk_work_order_material_consumptions_operator FOREIGN KEY (operator_id) REFERENCES employees(id) DEFERRABLE INITIALLY DEFERRED;
