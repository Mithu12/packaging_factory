...
import { Category, Subcategory, Supplier, Origin, Brand } from '@/types/inventory';
...
export function AddProductForm({
    ...
    if (open) {
      const fetchData = async () => {
        try {
          ...
          // Fetch categories
          const categoriesResponse = await fetch('/api/inventory/categories?limit=100', {
            credentials: 'include',
          });
          const categoriesData = await categoriesResponse.json();
          
          // Fetch brands
          const brandsResponse = await fetch('/api/inventory/brands?limit=100', {
            credentials: 'include',
          });
          const brandsData = await brandsResponse.json();
          
          // Fetch origins
          const originsResponse = await fetch('/api/inventory/origins?limit=100', {
            credentials: 'include',
          });
          const originsData = await originsResponse.json();
          
          // Fetch suppliers
          const suppliersResponse = await fetch('/api/inventory/suppliers?limit=100', {
            credentials: 'include',
          });
  ...
  const handleCategoryChange = async (categoryId: string) => {
    ...
    if (formData.name.trim()) {
      ...
      // Simple SKU generation for NextJS (frontend version has generateSKU utility)
      const namePart = formData.name.substring(0, 3).toUpperCase();
      const catPart = categoryName ? categoryName.substring(0, 3).toUpperCase() : '';
      const brandPart = brandName ? brandName.substring(0, 2).toUpperCase() : '';
      const randomPart = Math.floor(Math.random() * 1000).toString().padStart(3, '0');
      const generatedSKU = `${namePart}${catPart}${brandPart}${randomPart}`.replace(/\s/g, '');
    ...
    if (categoryId) {
      try {
        const subcategoriesResponse = await fetch(`/api/inventory/subcategories?category_id=${parseInt(categoryId)}&limit=100`, {
          credentials: 'include',
        });
  ...
  const handleInputChange = (field: keyof ProductFormData, value: string) => {
    ...
    if (field === "name" && value.trim()) {
      ...
      // Simple SKU generation (frontend version has generateSKU utility)
  ...
  const generateBarcodeFromSKUHandler = () => {
    ...
    // Simple barcode generation from SKU (frontend version has generateBarcodeFromSKU utility)
    const barcode = '978' + formData.sku.replace(/[^0-9]/g, '').padEnd(10, '0').substring(0, 10);
    setFormData((prev) => ({ ...prev, barcode: barcode }));
    toast.success("Barcode generated successfully!");
  };

  const generateRandomBarcode = () => {
    // Generate random barcode (frontend version has generateBarcode utility)
    const randomBarcode = '978' + Math.floor(Math.random() * 10000000000).toString().padStart(10, '0').substring(0, 10);
