// RBAC API service for Next.js app
import { UserWithPermissions, PermissionCheck, Role, Permission, RolesResponse, PermissionGroup, DepartmentStats } from '@/types/rbac';
...
export class RBACApi {
  ...
  static async getRoleById(roleId: number) {
    return makeRequest<Role & { permissions: Permission[]; user_count?: number }>(`/roles/${roleId}`);
  }
  ...
  static async updateRole(roleId: number, updateData: {
    ...
    return makeRequest<Role>(`/roles/${roleId}`, {
      method: 'PUT',
  ...
  static async deleteRole(roleId: number) {
    return makeRequest<{ id: number }>(`/roles/${roleId}`, {
      method: 'DELETE',
    });
  }
  ...
  static async getDepartmentStats() {
    return makeRequest<DepartmentStats[]>('/departments/stats');
  }
  ...
  static async getUsersWithPermission(permission: PermissionCheck) {
    ...
    }>(`/users/with-permission?${queryParams.toString()}`);
  }
